...
BencodeValue _decode_int(std::ifstream& file, std::string & raw_info, bool & _IH_parse_condition) {
...
BencodeValue _decode_bytestring(std::ifstream& file, std::string & raw_info, bool & _IH_parse_condition) {
  char c;
  ...
  while(str.size() != length && file.get(c)) {
   ...
   if (str == TF_String[INFO]) _IH_parse_condition = true;  // Begin reading infohash

  return BencodeValue{str};
}

BencodeValue decode(std::ifstream& file, std::string & raw_info, bool & _IH_parse_condition) {
  char c;
  file.get(c);
