...
const std::string
hexify_peer_id(const std::array<uint8_t, PEERID_SIZE> peerid) {
  std::stringstream ss;
...
void http_life(const std::shared_ptr<TrackerParams> &params) {
  ...
  for (;; std::this_thread::sleep_for(std::chrono::seconds(real_interval))) {
    ...
    if (std::holds_alternative<int64_t>(resp_dict[INTERVAL_STR])) {
      ...
      real_interval =
          interval ? interval : real_interval; // ensure non-zero interval
    }

    // std::cout << "interval: " << real_interval << std::endl;

    const std::string &peers_raw = std::get<std::string>(resp_dict[PEERS_STR]);

    for (size_t i = 0; i + 6 <= peers_raw.size(); i += 6) {
      uint32_t ip;
      uint16_t port;

      memcpy(&ip, &peers_raw[i], 4);
      memcpy(&port, &peers_raw[i + 4], 2);
